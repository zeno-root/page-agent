# Indofun AIGC Assistant Extension API

Integrate the Indofun AIGC Assistant extension into approved web apps and trigger multi-page browser tasks from page JavaScript.
...
- Internal Indofun AIGC extension package generated by `npm run build:ext`.
...
if (await waitForExtension()) {
    const result = await window.PAGE_AGENT_EXT!.execute('Click the login button', {
        baseURL: 'http://localhost:4800/api/page-agent/llm-proxy',
        apiKey: 'your-v18-login-token',
